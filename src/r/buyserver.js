...
export async function main(ns) {
	...
	if (ns.args[0] == 'delete' && ns.args[1] == 'all') {
		const resp = await ns.prompt('?! Confirm DELETE of ALL servers ?!');
		if (resp == false) {
	...
	if (ns.args[0] == 'delete') {
		const resp = await ns.prompt('Confirm DELETE of server named ' + ns.args[1]);
		if (resp == false) {
			ns.tprint("Transaction aborted.");
			ns.exit();
		}
	...
	if (ns.args[0] == '*') {
		...
		const resp = await ns.prompt('Confirm purchase of server named ' + ns.args[0] + ' with ' + FormatRam(ns, gb) + ' RAM for ' + FormatMoney(ns, ns.getPurchasedServerCost(2 ** pow)));
		if (resp == false) {
			ns.tprint("Transaction aborted.");
			return;
		}
