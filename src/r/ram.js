 ...
 * @param {any[]|undefined} params IDK
 * @param {bool|undefined} allowSpread
 * @param {bool|undefined} allowPartial
 */
export function RunScript(ns, scriptName, threads, params, allowSpread, allowPartial) {
  // Validate some preconditions
  if (typeof scriptName !== 'string' || scriptName.split('.').length < 2) {
    ns.tprint(`WARN: Invalid script name: ${scriptName}`);
    throw new Error('ERROR: Invalid script name provided.');
  }
  if (!scriptName.startsWith('r/')) {
    // ns.tprint(`Received invalid relative path for scriptName: ${scriptName}. Adding a prefix.`);
    scriptName = 'r/' + scriptName;
  }
  if (Number.isNaN(Number(threads)) || threads == 0) {
    ns.tprint(`WARN: Invalid # threads provided to RunScript: ${threads}. Settings to 1 and attempting to continue.`);
    threads = 1;
  }
  if (!Array.isArray(params)) {
    ns.tprint(`RunScript received invalid params. Overriding with empty array.: ${params}.`);
    params = [];
  }
  // Defaults and shared vars
  let ramMap = new MemoryMap(ns);
  // Find script RAM usage
  let ram = ns.getScriptRam(scriptName);
  // ns.tprint(`Script ram read resulted in: ${ram} for script: ${scriptName}`);
  ...
  while (fired < threads) {
    ...
    let coreBonus = 1;
    if (scriptName.startsWith('r/grow') || scriptName.startsWith('r/weaken')) {
